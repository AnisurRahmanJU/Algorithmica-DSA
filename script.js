...
function clearHost(){
  ...
  zoomContent.style.transform = 'scale('+APP.zoom+')';
  host.appendChild(zoomContent);
  return zoomContent;
}
...
/* ---------------------------------------------------------------------
   Zoom controls (buttons + ctrl-wheel + pinch) — works on desktop & mobile
--------------------------------------------------------------------- */
function applyZoomToDOM(){
  const zc = document.querySelector('.zoom-content');
  if(zc) zc.style.transform = 'scale('+APP.zoom+')';
...
function setZoom(z){
  APP.zoom = Math.max(0.5, Math.min(2.5, Math.round(z*100)/100));
  applyZoomToDOM();
  centerStageScroll();
}
function zoomIn(){ setZoom(APP.zoom + 0.15); }
function zoomOut(){ setZoom(APP.zoom - 0.15); }
...
/* ---------------------------------------------------------------------
   Stage zoom interactions: ctrl+wheel (desktop) and pinch (mobile)
--------------------------------------------------------------------- */
const stageEl = document.getElementById('stageEl');
stageEl.addEventListener('wheel', function(e){
  if(e.ctrlKey || e.metaKey){
    e.preventDefault();
    setZoom(APP.zoom - e.deltaY * 0.0015);
  }
}, {passive:false});
